...
documentControllers.controller('DocumentListCtrl', [
         '$scope',
         'Document',
         'Collections',
...
//                  console.log('lms:', doc.lms, '@', doc.id);
                  if (typeof doc.lms === 'undefined') {
//                     console.log('resetting lms @', doc.id);
                     doc.lms = [];
                  }
                  return doc;
               });
               ...
               for ( var c in search) {
                  if (search[c] != '') {
                     if (c != 'score') {
                        ...
                        switch (typeOf(x)) {
                        case 'array':
                           if (_.any(x, function(s) {
                              return s.match(new RegExp(search[c], 'i'));
                           }))
                              return true;
                           break
                        case 'string':
            ...
            $scope.highlighted_text = function(doc) {
               ...
               if (typeof text === 'undefined') {
               ...
               else if (typeof doc.lms === 'undefined') {
                  return text;
               }
               else {
                  // Collect all the segments
                  var hilites = []
                  //highest(doc.lms.filter(match)).forEach(
                  highest(doc.lms).forEach(
                           function(lm) {
                              hilites.push({
                                 type: 'source', pair: [lm.source.start, lm.source.end],
                                 text: null, name: lm.name
                              })
                              hilites.push({
                                 type: 'target', pair: [lm.target.start, lm.target.end],
                                 text: null, name: lm.name
                              })
                           });
                  ...
                  // console.log('**segments:', segments.map(function(s) {
                  // return
                  // s.text; }).join(' '));
                  // console.log('**compress:', compressed.map(function(s) {
                  // return
                  // s.text; }).join(' '));
            ...
            function makeFilter(search) {
               var filters = Object.keys(search).filter(function(c) {
                  return search[c].length > 0;
               }).map(function(c) {
                  var field = c.replace('_', '-'); // -> kebab-case
                  if (field.match(/score|.*coreness/)) {
                     var q = {
                        query: {
                           range: {}
                        }
            ...
            var do_query = function(startId) {
               ...
               var filter = makeFilter($scope.search)
               console.log('in do_query: query:', filter);
               var params = {
                  language: $scope.language.key, size: 20, startId: startId || ' '
               }
               var promise = Document.query(params, filter).$promise;
            ...
            var query_languages = function() {
               Collections.query({}).$promise.then(function(languages) {
...
// Single document
documentControllers.controller('DocumentItemCtrl', ['$scope', 'Document',
